...
def update_group(request: UpdateGroupRequest):
    ...
    new_group_id = g.group if g.group else g.id
...
def remove_from_group(id: int):
    """
    Handles removing an element from a group or reassigning leadership if needed.

    - If the element *belongs* to a group:
        * set its group field to NULL
        * update total_citations for the previous group leader

    - If the element does NOT belong to any group but *has members*
      (i.e. other records have group == id):
        * choose the member with the highest 'citations' as the new leader
        * reassign all other members to this new leader
        * clear (None) the old leader's total_citations
        * recalculate total_citations for the new leader

    - If the element has no group and no members:
        * no action is taken
    """
    db = SessionLocal()
    ...
    # Caso 1: l'elemento è membro di un gruppo -> rimuovilo dal gruppo
    if loc.group is not None:
        old_group_id = loc.group
        loc.group = None
        db.commit()
        update_total_citations(db, old_group_id)
        db.close()
        return {
            ...
            "message": f"Location {id} removed from group {old_group_id}",
            "old_group": old_group_id,
        }

    # Caso 2: l'elemento NON è in un gruppo; verifica se è leader di fatto (cioè ha membri)
    members = db.query(Location).filter(Location.group == loc.id).all()

    if not members:
        # Nessun gruppo in cui è membro, e nessun membro che appartenga a lui: niente da fare
        db.close()
        return {
            "status": "no_action",
            "message": f"Location {id} is not part of any group and has no members",
        }

    # Scegli come nuovo leader il membro con più citations
    new_leader = max(members, key=lambda m: m.citations)

    # Riassegna tutti i membri al nuovo leader
    reassigned = 0
    for m in members:
        if m.id == new_leader.id:
            continue
        m.group = new_leader.id
        reassigned += 1

    # Assicurati che il nuovo leader non sia in alcun gruppo
    new_leader.group = None
    new_leader_id = new_leader.id

    # Il vecchio leader perde il totale aggregato
    loc.total_citations = None

    db.commit()

    # Aggiorna i totali per il nuovo leader
    update_total_citations(db, new_leader.id)
    ...
    return {
        ...
        "members_reassigned": reassigned,
        "message": f"Location {id} had members; reassigned to new leader {new_leader_id} (max citations).",
    }
